...
function getHash(passcode, steps) { return MD5(passcode + steps.join('')).substr(0, 4) }

function isOpen(char) { return ['b', 'c', 'd', 'e', 'f'].includes(char) }
...
function getSteps(passcode) {
    ...
    while (positions.length > 0) {
        ...
        if (stepCounts[key] && stepCounts[key].length < position.steps.length) {
            continue
        } else {
            stepCounts[key] = position.steps
    ...
    Object.keys(stepCounts).filter(x => x.startsWith(target)).forEach(key => {
        const steps = stepCounts[key].join('')
