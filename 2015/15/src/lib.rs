...
fn read_ingredients(input: &Vec<String>) -> Vec<Ingredient> {
    ...
    for line in input {
        ...
        ingredients.push(Ingredient{capacity, durability, flavor, texture});
    }

    return ingredients;
}

fn calculate_score(indices: &Vec<usize>, ingredients: &Vec<Ingredient>) -> i64 {
    let mut capacity = 0;
    ...
    return cmp::max(0, capacity) * cmp::max(durability, 0) * cmp::max(flavor, 0) * cmp::max(texture, 0);
}

fn find_best_score<'a>(arr: &Vec<Vec<usize>>, indices: &mut Vec<usize>, index: usize, ingredients: &Vec<Ingredient>, best_score: &mut i64) {
    for (i, _) in arr[index].iter().enumerate() {
        indices[index] = i;
        if index == arr.len() - 1 {
            if indices.iter().sum::<usize>() == 100 {
                let score = calculate_score(indices, ingredients);
                if score > *best_score {
                    *best_score = score;
                }
            }
        ...
        if index < arr.len() - 1 {
            find_best_score(arr, indices, index + 1, ingredients, best_score);
        }
    }
}

pub fn science_for_hungry_people(input: &Vec<String>) -> i64 {
    ...
    find_best_score(&arr, &mut indices, 0, &ingredients, &mut best_score);
