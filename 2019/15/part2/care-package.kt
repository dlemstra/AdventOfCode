...
data class ValidDirection(val intcode: Intcode, val position: Position)

data class ValidPosition(val position: Position, val steps: Int)
...
class Position(var x: Int, var y: Int) {
    ...
    fun validNewDirections(intcode: Intcode, map: MutableMap<String, Int>): List<ValidDirection> {
        ...
        for (direction in (1L..4L)) {
            ...
            when (output) {
                ...
                1L -> result.add(ValidDirection(intcodeCopy, newPosition))
                2L -> {
                    map[newPosition.key] = 2
                    result.add(ValidDirection(intcodeCopy, newPosition))
                }
            }
        }
    ...
    fun validNewPositions(map: MutableMap<String, Int>, steps: Int): List<ValidPosition> {
        var result = mutableListOf<ValidPosition>()
        for (direction in (1L..4L)) {
            var newPosition = move(direction)

            if (map[newPosition.key] == 3)
                result.add(ValidPosition(newPosition, steps + 1))
        }

        return result
    }

    fun fillWithOxygen(map: MutableMap<String, Int>) {
        map["$x,$y"] = 2
    }
...
fun carePackage(data: String): Int {
    ...
    val validDirections = mutableListOf<ValidDirection>()
    ...
    while (true) {
        ...
        validDirections.addAll(position.validNewDirections(intcode, map))
    ...
    val startPosition = map.filterValues{ it == 2 }.keys.first().split(",")
    position = Position(startPosition[0].toInt(), startPosition[1].toInt())

    val validPositions = mutableListOf<ValidPosition>()

    var steps = 0
    var maxSteps = 0

    while (true) {
        position.fillWithOxygen(map)

        validPositions.addAll(position.validNewPositions(map, steps))

        if (validPositions.count() == 0)
            break

        var validPosition = validPositions.removeAt(0)
        position = validPosition.position
        steps = validPosition.steps
