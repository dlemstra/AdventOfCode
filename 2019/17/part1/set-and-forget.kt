...
fun getTotal(map: Map<String, Long>): Int {
    var total = 0
     for (position in map.keys) {
        val value = map[position] !!
        if (isScaffold(value)) {
            val x = position.split(",")[0].toInt()
            val y = position.split(",")[1].toInt()

            if ((isScaffold(map, x, y-1) && isScaffold(map, x, y+1) && isScaffold(map, x-1, y) && isScaffold(map, x+1, y)))
                total += x*y
        }
    }
...
fun setAndForget(data: String): Int {
