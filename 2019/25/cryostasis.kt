...
fun createInput(input: String): List<Long> = input.map{ it.toLong() } + 10L

fun printOutput(output: List<Long>) = println(output.map{ it.toChar() }.joinToString(""))

fun cryostasis(data: String) {
    val values = data.split(",").map{ it.toLong() }.toList()

    val intcode = Intcode(values)

    var output = intcode.execute()
    printOutput(output)

    val inputs = File("commands").readLines()
    for (input in inputs) {
        println(input)
        output = intcode.execute(createInput(input))
