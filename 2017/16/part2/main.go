...
func PermutationPromenade(actions []string, groupLength int, count int) string {
    group := createGroup(groupLength)

    start := strings.Join(group, "")
    groups := []string { start }

    newGroup := dance(group, actions)
    for start != newGroup {
        groups = append(groups, newGroup)
        newGroup = dance(group, actions)
    }

    index := count % len(groups)
...
func main() {
    ...
    result := PermutationPromenade(input, 16, 1000000000)
    fmt.Println(result)
}